...
function imprimirMayorValorInventario($arr){
    $mayorcantidad = 0;
    foreach($arr as $producto => $info){
        $preciox = ($info['cantidad']*$info['precio']);
        if($mayorcantidad < $preciox){
            $mayorcantidad = $preciox;
        }
    }
    return $mayorcantidad;
}

echo "<br><br> El mayor valor total en inventario es de : ".imprimirMayorValorInventario($inventario);
