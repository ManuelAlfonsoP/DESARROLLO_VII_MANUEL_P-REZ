...
function generarReporteBiblioteca($biblioteca) {
    ...
    $prestados = count(array_filter($biblioteca, function($libro) {
        return $libro['prestado'] === true;
    }));

    $generos = array_count_values(array_column($biblioteca, 'genero'));

    $autores = array_count_values(array_column($biblioteca, 'autor'));

    arsort($autores);
    $autorConMasLibros = key($autores);
    
    // Devolver el reporte como un array asociativo
    return [
        "total_libros" => $totalLibros,
        "libros_prestados" => $prestados,
        "libros_por_genero" => $generos,
