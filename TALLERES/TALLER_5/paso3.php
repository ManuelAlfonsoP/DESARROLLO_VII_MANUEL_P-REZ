...
function EstudiantesparaTutoria($arr){
    return array_filter($arr, function($a){
        return $a["promedio"] < 75;
    });
}

function EstudiantesparaHonor($arr){
    return array_filter($arr, function($b){
        return $b["promedio"] >= 90;
    });
}

echo "<br><br> Estudiantes que necesitan tutoria: ";
foreach (EstudiantesparaTutoria($estudiantes) as $e){
    echo "- {$e['nombre']} (promedio: " . number_format($e['promedio'], 2) . ")<br>";
}
echo "<br> Estudiantes para codigo de honor: ";
foreach (EstudiantesparaHonor($estudiantes) as $e){
    echo "- {$e['nombre']} (promedio: " . number_format($e['promedio'], 2) . ")<br>";
}
